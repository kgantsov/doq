...
func (n *Node) Backup(w io.Writer, since uint64) (uint64, error) {
	log.Info().
		...
		Msgf("Backing up database from version %d", since)
	...
	log.Info().
		Str("component", "backup").
		Msgf("Database backed up from version %d to %d", since, maxVersion)
...
func (n *Node) Restore(r io.Reader, maxPendingWrites int) error {
	log.Info().
		Str("component", "backup").
		Msgf("Restoring database from a file wih maxPendingWrites %d", maxPendingWrites)
	...
	log.Info().
		Str("component", "backup").
		Msgf("Database restored successfully with maxPendingWrites %d", maxPendingWrites)
	return nil
}
