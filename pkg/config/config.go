...
func InitCobraCommand(runFunc func(cmd *cobra.Command, args []string)) *cobra.Command {
	...
	rootCmd.PersistentFlags().String("storage.data_dir", "data", "Data directory")
	rootCmd.PersistentFlags().Int64("storage.gc_interval", 300, "Garbage collection interval in seconds")
	rootCmd.PersistentFlags().Float64("storage.gc_discard_ratio", 0.7, "Garbage collection discard ratio")
	rootCmd.PersistentFlags().String("storage.compression", "", "Compression type. Options: none, snappy, zstd")
	rootCmd.PersistentFlags().Int("storage.zstd_compression_level", 0, "ZSTD compression level")
	rootCmd.PersistentFlags().Int64("storage.block_cache_size", 0, "Block cache size in MB")
	rootCmd.PersistentFlags().Int64("storage.index_cache_size", 0, "Index cache size in MB")
	rootCmd.PersistentFlags().Int64("storage.base_table_size", 0, "Base table size in bytes")
	rootCmd.PersistentFlags().Int64("storage.base_level_size", 0, "Maximum size target for the base level")
	rootCmd.PersistentFlags().Int("storage.num_compactors", 0, "Number of compaction workers")
	rootCmd.PersistentFlags().Int("storage.num_level_zero_tables", 0, "Number of level zero tables")
	rootCmd.PersistentFlags().Int("storage.num_level_zero_tables_stall", 0, "Number of level zero tables to stall compaction")
	rootCmd.PersistentFlags().Int("storage.num_memtables", 0, "Number of memtables")
	rootCmd.PersistentFlags().Int64("storage.value_log_file_size", 0, "Value log file size in bytes")
	...
	viper.BindPFlag("storage.data_dir", rootCmd.PersistentFlags().Lookup("storage.data_dir"))
	viper.BindPFlag("storage.gc_interval", rootCmd.PersistentFlags().Lookup("storage.gc_interval"))
	viper.BindPFlag("storage.gc_discard_ratio", rootCmd.PersistentFlags().Lookup("storage.gc_discard_ratio"))
	viper.BindPFlag("storage.compression", rootCmd.PersistentFlags().Lookup("storage.compression"))
	viper.BindPFlag("storage.zstd_compression_level", rootCmd.PersistentFlags().Lookup("storage.zstd_compression_level"))
	viper.BindPFlag("storage.block_cache_size", rootCmd.PersistentFlags().Lookup("storage.block_cache_size"))
	viper.BindPFlag("storage.index_cache_size", rootCmd.PersistentFlags().Lookup("storage.index_cache_size"))
	viper.BindPFlag("storage.base_table_size", rootCmd.PersistentFlags().Lookup("storage.base_table_size"))
	viper.BindPFlag("storage.base_level_size", rootCmd.PersistentFlags().Lookup("storage.base_level_size"))
	viper.BindPFlag("storage.num_compactors", rootCmd.PersistentFlags().Lookup("storage.num_compactors"))
	viper.BindPFlag("storage.num_level_zero_tables", rootCmd.PersistentFlags().Lookup("storage.num_level_zero_tables"))
	viper.BindPFlag("storage.num_level_zero_tables_stall", rootCmd.PersistentFlags().Lookup("storage.num_level_zero_tables_stall"))
	viper.BindPFlag("storage.num_memtables", rootCmd.PersistentFlags().Lookup("storage.num_memtables"))
	viper.BindPFlag("storage.value_log_file_size", rootCmd.PersistentFlags().Lookup("storage.value_log_file_size"))
